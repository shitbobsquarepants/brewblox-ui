...
const lastUpdate = computed(() =>
  agitator.value ? formatTimestamp(agitator.value.timestamp) : '',
);
const agitatorOpts = computed(() =>
  store.allAgitators.map((a) => ({ label: a.name, value: a.device_id })),
);
  ...
  <Card>
      ...
      <template v-else>
          ...
          >
            ...
            <g :transform="'translate(78, 40) rotate(' + angle + ')'">
              <line
                ...
                x2="0"
                y2="-12"
                :stroke="agitator?.running ? '#60a5fa' : '#444'"
                stroke-width="3"
        ...
        <template v-if="config.hasVFD && agitator">
          <div class="row justify-center items-baseline q-mb-xs">
            <span class="text-h4 text-bold text-mono">{{
              agitator.speed_rpm?.toFixed(0) ?? '---'
            }}</span>
          ...
          <div class="q-px-sm q-mb-xs">
            <q-linear-progress
              :value="speedPct / 100"
              :color="agitator.running ? 'blue-4' : 'grey-7'"
              rounded
              size="6px"
            />
    ...
    >
      <SelectField
        ...
        label="Agitateur"
        :options="agitatorOpts"
        @update:model-value="(v) => patchConfig({ deviceId: v })"
      />
      <LabeledField label="Variateur de frequence (VFD)">
        <q-toggle
          :model-value="config.hasVFD"
          @update:model-value="(v) => patchConfig({ hasVFD: v })"
        />
      </LabeledField>
      <LabeledField
        v-if="config.hasVFD"
        label="Vitesse max (rpm)"
      >
        <q-input
          :model-value="config.maxRpm"
          type="number"
          dense
          borderless
          @update:model-value="(v) => patchConfig({ maxRpm: Number(v) })"
        />
      </LabeledField>
    </div>
  </Card>
