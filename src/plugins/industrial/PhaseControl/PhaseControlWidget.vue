...
const stepProgress = computed(() => {
  ...
  return Math.min(
    1,
    phase.value.step_elapsed_seconds / phase.value.step_duration_seconds,
  );
});
...
const canStart = computed(
  () =>
    !phase.value ||
    ['IDLE', 'COMPLETED', 'KILLED', 'ERROR'].includes(phase.value.status),
);
...
const canKill = computed(
  () =>
    phase.value &&
    !['IDLE', 'COMPLETED', 'KILLED'].includes(phase.value.status),
);
...
<template>
  <Card>
      ...
      <template v-else>
        ...
        >
          <div class="row items-center justify-between">
            <div class="row items-center">
              ...
              <span class="text-bold text-white q-mr-sm">{{
                phase?.current_step ?? '--'
              }}</span>
            ...
            <span
              ...
              :class="
                phase?.status === 'RUNNING' ? 'text-positive' : 'text-grey'
              "
            >
              {{ stepTimer }}
            </span>
        ...
        <div class="row justify-between items-center q-mb-sm">
          <q-btn
            ...
            label="Step + 1"
            size="sm"
            @click="sendCommand('step_next')"
          />
          <span class="text-caption text-grey">{{ lastUpdate }}</span>
        </div>

        <div
          v-if="phase?.selections?.length"
          class="q-mt-sm"
        >
          <div
            v-for="sel in phase.selections"
            :key="sel.id"
            class="q-mb-xs"
          >
    ...
    <div
      ...
      class="widget-body column q-pa-sm"
    >
      <SelectField
      ...
      <div class="row q-col-gutter-sm">
        <div class="col-6">
          <LabeledField label="Label PV1"
            ><q-input
              :model-value="config.pv1Label"
              dense
              borderless
              @update:model-value="(v) => patchConfig({ pv1Label: String(v) })"
          /></LabeledField>
        </div>
        <div class="col-6">
          <LabeledField label="Unite PV1"
            ><q-input
              :model-value="config.pv1Unit"
              dense
              borderless
              @update:model-value="(v) => patchConfig({ pv1Unit: String(v) })"
          /></LabeledField>
        </div>
        <div class="col-6">
          <LabeledField label="Label SP1"
            ><q-input
              :model-value="config.sp1Label"
              dense
              borderless
              @update:model-value="(v) => patchConfig({ sp1Label: String(v) })"
          /></LabeledField>
        </div>
        <div class="col-6">
          <LabeledField label="Label PV2"
            ><q-input
              :model-value="config.pv2Label"
              dense
              borderless
              @update:model-value="(v) => patchConfig({ pv2Label: String(v) })"
          /></LabeledField>
        </div>
        <div class="col-6">
          <LabeledField label="Unite PV2"
            ><q-input
              :model-value="config.pv2Unit"
              dense
              borderless
              @update:model-value="(v) => patchConfig({ pv2Unit: String(v) })"
          /></LabeledField>
        </div>
        <div class="col-6">
          <LabeledField label="Label SP2"
            ><q-input
              :model-value="config.sp2Label"
              dense
              borderless
              @update:model-value="(v) => patchConfig({ sp2Label: String(v) })"
          /></LabeledField>
        </div>
      </div>
    </div>
