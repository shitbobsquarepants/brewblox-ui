...
const hasFault = computed(() =>
  agitator.value?.status === 'FAULT' || agitator.value?.status === 'COMM_ERROR',
);
...
watch(isRunning, (v) => { if (v) startAnim(); else stopAnim(); }, { immediate: true });
...
<template>
  ...
  >
    ...
    <circle cx="25" cy="25" r="20"
      ...
      stroke-width="2"
    />
    <text x="25" y="31" text-anchor="middle"
      fill="white" font-size="16" font-weight="bold" font-family="monospace"
    >M</text>

    <!-- Indicateur état -->
    <circle cx="40" cy="10" r="6" :fill="indicatorColor" />
    <text v-if="hasFault" x="40" y="14"
      text-anchor="middle" fill="white" font-size="7" font-weight="bold"
    >!</text>

    <!-- Arbre -->
    <line x1="25" y1="45" x2="25" y2="58"
        ...
        <q-list>
          ...
          <q-item>
            <q-item-section class="text-caption">
              ...
              {{ agitator?.speed_rpm ? '— ' + agitator.speed_rpm.toFixed(0) + ' rpm' : '' }}
            </q-item-section>
          </q-item>
        </q-list>
      </q-menu>
